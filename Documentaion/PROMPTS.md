...
**Purpose:** Allocate the 45 prompts / 28 phases above across **~15 Claude Free accounts**, so the team can build CHAINVISION in parallel over the 5-day window without merge conflicts or blocked idle time. Each "account" below is a persistent Claude session that a human coordinator re-opens across the 5 days as its dependencies unlock — it is not a single-use, single-prompt session. Grouping prompts this way (rather than 1 prompt = 1 account) keeps related files with the person/session who already has context on them, which is the single biggest lever for reducing integration bugs in a 15-way parallel build.

**How this differs from the seed division in the request:** The seed 15-role list is preserved as the backbone (each account below still maps to one of the 15 named roles), but four adjustments were made for correctness and load-balancing:
1. **Backend Foundation (Account 4)** was expanded to include both P1(Node) *and* PR2(Java) scaffolding/foundation *and* their Phase 24 error-handling hardening, because those prompts all touch the same small set of infra files (`index.ts`, `application.yml`, `errorHandler.ts`, `GlobalExceptionHandler.java`) that this account already owns — splitting them across accounts would just create merge risk for no parallelism benefit.
2. **P1 Calculation Engine (Account 5)** absorbs the two prompts that *modify* `recommendationService.ts` after Phase 7 creates it (P20.1 the P1→PR2 handoff call, P22.1 the Gemini rationale call), even though those prompts are officially filed under Phase 20/22. `recommendationService.ts` is edited three times across the project (P7.2 creates it, P20.1 adds one call, P22.1 adds another) — keeping one owner for the whole file's lifetime avoids three different accounts fighting over the same 40-line file.
3. **Gemini NLP (Account 9)** and **Tesseract OCR (Account 11)** are single-prompt roles on the seed list, so each was given a natural Day-4/5 follow-up (P28.2 presentation content for the AI story; P25.3 the end-to-end smoke test) instead of sitting idle after Day 2.
4. **Testing prompts (P25.1, P25.2)** were reassigned from a generic "Account 14 testing" bucket to the accounts that already own the engines under test (Account 5 owns the P1 engines it's testing; Account 8 owns the PR2 engines it's testing) — the seed list's "Authentication/testing" combination was split so Account 14 stays focused on Auth + the one frontend cross-cutting prompt (P24.3) that also touches `App.tsx`.

All 45 prompts are assigned exactly once below; nothing is duplicated or dropped.

---

## MASTER EXECUTION ORDER

This is the strict phase-completion order the whole project is gated on, independent of which account runs which prompt. Numbers are Phase numbers; letters mark work that can run alongside the phase above it.

```
Day 1
  Phase 1  (P1.1)                          — must land first, blocks everything
  Phase 1  (P1.2 ∥ P1.3 ∥ P1.4)            — parallel, each blocks its own stack only
  Phase 2  (P2.1)                          — blocks Phase 3, 4, and all data work

Day 2
  Phase 3  (P3.1 ∥ P3.2)                   — needs P2.1
  Phase 4  (P4.1 → P4.2)                   — needs P1.3/P1.4 + P2.1; P4.2 needs P4.1's contract file
  Phase 5  (P5.1)                          — needs P4.1, P3.1
  Phase 6  (P6.1)                          — needs P4.1, P3.1 (parallel with P5.1)
  Phase 10 (P10.1)                         — needs P4.2, P3.2
  Phase 11 (P11.1)                         — needs P4.2, P3.2 (parallel with P10.1)
  Phase 15 (P15.1)                         — needs P4.1

Day 3
  Phase 7  (P7.1 → P7.2)                   — needs P5.1 + P6.1
  Phase 8  (P8.1)                          — needs P7.2
  Phase 9  (P9.1 → P9.2)                   — needs P1.2, P8.1
  Phase 12 (P12.1)                         — needs P11.1
  Phase 13 (P13.1)                         — needs P10.1, P11.1
  Phase 14 (P14.1)                         — needs P13.1
  Phase 16 (P16.1)                         — needs P14.1, P15.1
  Phase 19 (P19.1 → P19.2)                 — needs P1.2/P11.1, then +P12.1/P13.1/P14.1

Day 4
  Phase 17 (P17.1)                         — needs P16.1
  Phase 18 (P18.1)                         — needs P17.1
  Phase 19 (P19.3)                         — needs P16.1, P17.1, P18.1
  Phase 20 (P20.1)                         — needs P8.1, P11.1
  Phase 21 (P21.1)                         — needs P9.2, P19.2, P19.3
  Phase 22 (P22.1)                         — needs P7.2, P20.1
  Phase 23 (P23.1 ∥ P23.2 ∥ P23.3)         — needs P2.1 + (P9.1/P19.1 for .1; P4.1 for .2; P4.2 for .3)
  Phase 24 (P24.1 ∥ P24.2 ∥ P24.3)         — needs Phase 22, 15/16/17, 9.2/19.2/19.3 respectively
  Phase 25 (P25.1 ∥ P25.2)                 — needs the relevant engines complete

Day 5
  Phase 25 (P25.3)                         — needs P20.1, P21.1, P22.1, P18.1, P19.3 (everything functional)
  Phase 26 (P26.1)                         — needs P25.3
  Phase 27 (P27.1 ∥ P27.2)                 — needs P26.1
  Phase 28 (P28.1 → P28.2)                 — needs P26.1/P27.1, then P28.1
```
...
Five streams run mostly independently once their Day-1/2 prerequisites land. Cross-stream syncs are the only points that need coordinator attention.

| Stream | Accounts | What it owns | Syncs with |
|---|---|---|---|
| **A — Infra/Platform** | 1, 2, 4 | Repo skeleton, Supabase schema, both backends' config/datasource/error-handling shells | Everyone reads from this stream; it writes to almost nothing after Day 2 |
| **B — P1 (Demand & Replenishment)** | 3 (p1 half), 5, 6, 7, 13 | Deterministic engines, recommendation generator, P1 APIs, Planner frontend, Realtime hook creation | Hands the recommendation object to Stream C at Phase 20 |
| **C — PR2 (Procure-to-Pay)** | 3 (pr2 half), 8, 9, 10, 11, 12 | Supplier scoring, requisition/PO/GRN, Gemini intent + structuring, OCR, 3-way match, payment, Procurement frontend | Receives from Stream B at Phase 20; feeds Stream D |
| **D — Cross-cutting hardening** | 14 | Auth (frontend + both backends' JWT verification), frontend error boundaries | Runs after both frontends have real pages (Phase 9/19 done) |
| **E — Quality/Ship** | 15 | Bug bash, deployment, demo rehearsal, presentation outline | Runs last, consumes everything |

Within Stream B, Accounts 5, 6, 7, 13 fan out from Account 4's Phase 4 handoff. Within Stream C, Accounts 8, 9, 11, 12, 10 fan out from Account 4's Phase 4 handoff on the Java side. The two streams only touch each other at exactly two points: the P1→PR2 handoff (Phase 20) and the shared Supabase Realtime tables (Phase 21) — everything else is genuinely parallel.
...
```
              ...
              ┌────────────────────────┼────────────────────────┐
              ▼                        ▼                        ▼
        P1.2 (Acct 7)            P1.3 (Acct 4)            P1.4 (Acct 4)
      [frontend scaffold]      [P1 node scaffold]       [PR2 java scaffold]
              │                        │                        │
              │                        └──────────┐  ┌───────────┘
              │                                    ▼  ▼
              │                              P2.1 (Acct 2) [Supabase schema]
              │                                    │
              │                    ┌───────────────┼───────────────┐
              │                    ▼                                ▼
              │              P3.1 (Acct 3)                    P3.2 (Acct 3)
              │            [p1 seed data]                  [pr2 seed data]
              │                    │                                │
              │                    ▼                                ▼
              │              P4.1 (Acct 4)                    P4.2 (Acct 4)
              │         [P1 foundation+contract]        [PR2 foundation+DTO]
              │              │        │                        │        │
              │              ▼        ▼                        ▼        ▼
              │        P5.1(A5)  P6.1(A5)              P10.1(A8)  P11.1(A8)
              │      [demand]  [inventory/expiry]     [supplier]  [requisition]
              │              │        │                        │        │
              │              └───┬────┘                        │        ▼
              │                  ▼                              │  P12.1 (Acct 9)
              │            P7.1 (Acct 5)                        │  [Gemini intent]
              │          [FEFO + ROP/EOQ]                       │        │
              │                  ▼                    ┌─────────┴────────┘
              │            P7.2 (Acct 5)               ▼
              │          [recommendation gen]     P13.1 (Acct 8) [PO]
              │                  ▼                     ▼
              │            P8.1 (Acct 6)          P14.1 (Acct 8) [GRN]
              │           [P1 REST APIs]               │
              │                  │              P15.1 (Acct 11) [Tesseract OCR, on P1]
              │                  │                     │        │
              │                  │                     └────┬───┘
              │                  │                          ▼
              │                  │                    P16.1 (Acct 12)
              ...
              │                  └──────────┬───────────────┘
              │                             ▼
              │                    P20.1 (Acct 5) [P1→PR2 handoff]
              │                             ▼
              │                    P22.1 (Acct 5) [Gemini rationale]
              │
        ┌─────┴─────┐
        ▼           ▼
   P9.1(A7)    P19.1(A10)
   [planner     [procurement
    shell]       shell]
        ▼           ▼
   P9.2(A7)    P19.2(A10) ──needs P12.1,P13.1,P14.1
        │           ▼
        │      P19.3(A10) ──needs P16.1,P17.1,P18.1
        │           │
        └─────┬─────┘
              ▼
        P21.1 (Acct 13) [Supabase Realtime]
              ▼
   P23.1(A14) → P23.2(A14) ∥ P23.3(A14)   [Auth: frontend, P1, PR2]
              ▼
   P24.1(A4) ∥ P24.2(A4) ∥ P24.3(A14)     [error handling/fallbacks]
              ▼
   P25.1(A5) ∥ P25.2(A8) ∥ P25.3(A11)     [test gap-fill + smoke test]
              ▼
        P26.1 (Acct 15) [bug bash]
              ▼
   P27.1(A1) ∥ P27.2(A15)                 [deployment]
              ▼
   P28.1(A15) → P28.2(A9)                 [demo rehearsal → presentation]
```
...
One row per major path/module. "Owner" = the account that creates and modifies it across the project; other accounts listed under "Readers" may only read it (per each prompt's `READ ONLY:` list — see Global Rule #2/#3).

| Path | Owner (Account) | Readers | Notes |
|---|---|---|---|
| `/README.md`, `/.gitignore`, `/docker-compose.yml`, `/.env.example` | 1 | all | Created P1.1, filled in piecemeal by every scaffold prompt's `MODIFY:` list, finalized P27.1. Treat `.env.example`/`docker-compose.yml` as append-only from other accounts — never remove another account's block. |
| `/infra/supabase/migrations/**`, `/infra/supabase/README.md` | 2 | all backend accounts | Frozen after P2.1 — later prompts read column names, never alter schema. |
| `/shared/seed-data/**` | 3 | 5, 8, 11, 15 (read for testing) | `p1_seed_data.json` is read-only for Account 3's own PR2 seed script (P3.2) once written. |
| `/shared/contracts/**` | 4 (P4.1 authors the JSON Schema) | 5, 8 | PR2's DTO mirror (`ReplenishmentRecommendationDto.java`) is owned by Account 4 too (P4.2), so the contract and its two mirrors never drift. |
| `/p1-backend/src/index.ts`, `/config/`, `/db/`, `/middleware/` | 4 | 5, 6, 11, 13, 14 | Account 4 owns the *mounting* of routes; route-file authors (5, 6, 15) add one line each to `index.ts` per their own prompt — coordinate through the coordinator if two land the same day. |
| `/p1-backend/src/engine/**` | 5 | 6, 25.1 self | Pure, no-I/O files — lowest conflict risk in the repo. |
| `/p1-backend/src/services/demandService.ts`, `inventoryService.ts`, `recommendationService.ts`, `pr2ClientService.ts`, `geminiClient.ts`, `aiRationaleService.ts` | 5 | 6, 13 | `recommendationService.ts` is edited three times (P7.2, P20.1, P22.1) — same owner throughout by design (see note above). |
| `/p1-backend/src/routes/**` | 6 | 7, 9(frontend), 23(auth) | P23.2 adds middleware to these files but does not touch handler logic — small additive diff only. |
| `/p1-backend/src/services/ocrService.ts`, `/routes/internalOcrRoutes.ts` | 11 | 4, 12 | Account 4 (P24.1) makes a small additive error-handling change here after Account 11's P15.1 lands. |
| `/pr2-backend/.../config/`, `DatasourceConfig`, `GlobalExceptionHandler` | 4 | all PR2 accounts | Mirrors the P1-side foundation ownership pattern. |
| `/pr2-backend/.../sourcing/**` | 8 | 13 (PO) | `SupplierScoringEngine` is unit-tested by Account 8 itself in P25.2. |
| `/pr2-backend/.../requisition/**` | 8 | 9 (adds `IntentExtractionService` + one controller route) | Account 8 leaves the `// TODO: wired by Phase 12` extension point for Account 9. |
| `/pr2-backend/.../purchaseorder/**`, `/goodsreceipt/**` | 8 | 12 | |
| `/pr2-backend/.../ai/GeminiClient.java` | 9 | 4 (P24.2 wraps it with retry), 12 (reads it for invoice structuring + mismatch explanation) | Single shared AI client — never duplicate a second Gemini wrapper. |
| `/pr2-backend/.../invoice/**`, `/payment/**` | 12 | 4 (P24.2), 8 | `ThreeWayMatchController.java` gets one additive line from Account 12's own P18.1 (calls `PaymentApprovalService`) — same account, no handoff risk. |
| `/pr2-backend/.../security/**` | 14 | — | Filter-chain only; never edits any `*Controller.java` directly (enforced by P23.3's own constraints). |
| `/frontend/src/pages/planner/**`, `p1Client.ts` | 7 | 13 (adds realtime hook usage), 14 (swaps one import line) | |
| `/frontend/src/pages/procurement/**`, `pr2Client.ts` | 10 | 13, 14 | |
| `/frontend/src/hooks/useAuthStub.ts` → `useAuth.ts`, `RequireRole.tsx`, `LoginPage.tsx` | 14 | 7, 10 (import-line swap only) | `useAuthStub.ts` is left in place, unused, until Phase 28 cleanup. |
| `/frontend/src/lib/supabaseClient.ts`, `useRealtimeTable.ts` | 13 | 14 (reuses the client, does not duplicate it) | |
| `/frontend/src/components/ErrorBoundary.tsx`, `Toast.tsx`, `apiInterceptor.ts` | 14 | — | Designed to need zero per-page edits, per P24.3's own constraint. |
| `/frontend/src/App.tsx` | **Shared, sequential** — 7 (P9.1/P9.2 routes) → 10 (P19.1–19.3 routes) → 14 (P23.1 guards, P24.3 error boundary wrap) | all frontend accounts | The one genuinely hot file in the repo. Each prompt's change is additive (new route block / new wrapper), but land these in the stated order to avoid a 3-way merge conflict. |
| `/scripts/**`, `/INTEGRATION_BUG_BASH.md`, `/DEPLOYMENT.md`, `/DEMO_SCRIPT.md`, `/PRESENTATION_OUTLINE.md` | 11 (`e2e_smoke_test.ts`), 15 (everything else) | all | Read-only reference for everyone during Phase 26. |
...
**Prompts:** P1.1, P27.1
**Dependencies:** None (P1.1 is the project's root); P27.1 needs P26.1 complete
**Files owned:** `/README.md`, `/.gitignore`, `/.env.example`, `/docker-compose.yml`, top-level `.gitkeep` scaffolding
**When to start:** Immediately, Day 1 hour 0. This is the single account every other account is blocked on — treat it as the highest-priority kickoff.
**Expected output:** Day 1 — a valid, empty monorepo skeleton (`docker compose config` passes, no secrets committed) that unblocks all 14 other accounts within the first hour. Day 5 — the finalized `docker-compose.yml`, a real `README.md` replacing the Day-1 placeholder, and a fully audited `.env.example`.

---

## ACCOUNT 2
**Role:** Supabase Schema
**Prompts:** P2.1
**Dependencies:** P1.1
**Files owned:** `/infra/supabase/migrations/0001–0004*.sql`, `/infra/supabase/README.md`
**When to start:** Day 1, as soon as P1.1 lands (do not wait for P1.2/1.3/1.4 — schema work is independent of app scaffolding).
**Expected output:** Day 1 — every table from Section 7 (both `p1`/`pr2` schemas plus shared `users`), RLS enabled, Realtime replication enabled on the six tables Phase 21 needs, migrations re-runnable on a clean project. This is the second hardest blocker in the project after P1.1 — nine other accounts (3, 4, 5, 6, 8, 10, 12, 13, 14) are directly or transitively waiting on it. After Day 1, this account is on standby through Day 5 to answer schema questions and patch migrations if Phase 26's bug bash surfaces a column/type mismatch.
...
**Prompts:** P3.1, P3.2
**Dependencies:** P2.1
**Files owned:** `/shared/seed-data/**` (both P1 and PR2 datasets, sample invoice files, both READMEs)
**When to start:** Day 1 late / Day 2 early, as soon as P2.1's migrations are applied.
**Expected output:** Day 2 — 15–20 SKUs, 4–6 DCs, batch-level inventory with near-expiry and critical batches, 90 days of demand history with the seasonal spike, 3–5 pre-seeded low-stock combos (Phase 5–7 depend on this); 6–10 suppliers with a deliberate "cheap/unreliable" vs "expensive/reliable" spread, pre-existing POs/GRNs, and the 3 sample invoice files (matching, qty-mismatch, price-mismatch) that Phases 15–18 and the whole second half of the demo script depend on.

---

## ACCOUNT 4
**Role:** Backend Foundation
**Prompts:** P1.3, P1.4, P4.1, P4.2, P24.1, P24.2
**Dependencies:** P1.1 (for P1.3/P1.4); P1.3+P2.1 (for P4.1); P1.4+P2.1+P4.1 (for P4.2); P22.1+P15.1 (for P24.1); P12.1+P16.1+P17.1 (for P24.2)
**Files owned:** `/p1-backend/src/index.ts`, `/config/env.ts`, `/db/supabaseClient.ts`, `/middleware/errorHandler.ts`, `rateLimitAwareRetry.ts`; `/pr2-backend` `pom.xml`/`application.yml`/`DatasourceConfig.java`/`GlobalExceptionHandler.java`/`RateLimitAwareRetry.java`; both Dockerfiles
**When to start:** Day 1, immediately after P1.1 (does not need to wait on Account 1's later work). Returns Day 4 for the two Phase 24 hardening prompts.
**Expected output:** Day 1 — both backend skeletons booting with a working `/health` route each. Day 2 — both backends wired to Supabase (service-role client on P1, JDBC datasource on PR2), the `ReplenishmentRecommendation` JSON Schema and its Java DTO mirror published, unblocking 8 downstream accounts (5, 6, 8, 9, 11, 12). Day 4 — consistent `{error:{code,message}}` shapes on both APIs, Gemini/OCR calls wrapped in retry-with-backoff, no unhandled crashes on simulated failures.

---

## ACCOUNT 5
**Role:** P1 Calculation Engine
**Prompts:** P5.1, P6.1, P7.1, P7.2, P20.1, P22.1, P25.1
**Dependencies:** P4.1 + P3.1 (for P5.1/P6.1); P5.1+P6.1 (for P7.1); P7.1 (for P7.2); P8.1+P11.1 (for P20.1); P7.2+P20.1 (for P22.1); all four engines complete (for P25.1)
**Files owned:** `/p1-backend/src/engine/**` (demand, expiry risk, FEFO, replenishment math — all pure/no-I/O), `/p1-backend/src/services/demandService.ts`, `inventoryService.ts`, `recommendationService.ts`, `pr2ClientService.ts`, `geminiClient.ts` (P1 side), `aiRationaleService.ts`
**When to start:** Day 2, as soon as P4.1 and P3.1 land. This account owns the project's deterministic core (Section 5.1's "no AI in the numeric path" rule) end-to-end, so keeping it on one account is deliberate.
**Expected output:** Day 2–3 — four pure, unit-tested engines (demand, expiry risk, FEFO, safety-stock/ROP/EOQ) with all formulas/constants documented in code comments, plus the recommendation generator producing correctly-shaped rows for the seeded low-stock SKUs. Day 4 — the live P1→PR2 handoff (retry-once on failure, LOW-urgency excluded) and Gemini-generated rationale text (with a genuinely informative deterministic fallback) both wired into the same `recommendationService.ts`. Day 4 late — an expanded, documented unit-test suite across all four engines.
...
**Prompts:** P8.1
**Dependencies:** P7.2
**Files owned:** `/p1-backend/src/routes/replenishmentRoutes.ts`, `skuRoutes.ts`, `dcRoutes.ts`, `/p1-backend/tests/api.integration.test.ts`
**When to start:** Day 2 late / Day 3 early, as soon as P7.2 lands.
**Expected output:** Day 3 — every P1 route from Section 13.1 mounted and integration-tested (including 404 handling), unblocking Account 7 (frontend) and Account 5's own P20.1. This is a small, single-prompt account — from Day 3 onward it is available as extra bug-fix capacity for Account 15 during the Day 4–5 integration bug bash, specifically for anything at the P1 API layer.

---

## ACCOUNT 7
**Role:** P1 Frontend
**Prompts:** P1.2, P9.1, P9.2
**Dependencies:** P1.1 (for P1.2); P1.2+P8.1 (for P9.1); P9.1 (for P9.2)
**Files owned:** `/frontend/package.json`/`vite.config.ts`/`tailwind.config.js`/Dockerfile, `/frontend/src/main.tsx`, `App.tsx` (Planner routes only — see shared-file note), `/frontend/src/pages/planner/**`, `/frontend/src/api/p1Client.ts`, `/frontend/src/hooks/useAuthStub.ts`, `/frontend/src/components/NavBar.tsx`
**When to start:** Day 1, in parallel with P1.3/P1.4 (only needs P1.1). Since this account also scaffolds the whole `/frontend` tree, coordinate with Account 10 before Day 1 ends so Account 10 knows the routing library choice (`react-router-dom`) and shared auth-stub contract before it starts P19.1 on Day 3.
**Expected output:** Day 1 — a working Vite/React/Tailwind dev server with a placeholder page. Day 3 — the Planner dashboard shell plus the three real content views (inventory table, expiry heatmap, recommendations list) and the Demand Spike Simulator that drives Section 16 step 2 of the live demo — the project's opening screen.

---

## ACCOUNT 8
**Role:** PR2 Business Logic
**Prompts:** P10.1, P11.1, P13.1, P14.1, P25.2
**Dependencies:** P4.2 + P3.2 (for P10.1/P11.1); P10.1+P11.1 (for P13.1); P13.1 (for P14.1); P10.1+P17.1 complete (for P25.2)
**Files owned:** `/pr2-backend/.../sourcing/**` (Supplier entity, repository, scoring engine, controller), `/pr2-backend/.../requisition/**` (entity, repository, service, controller — with the extension point left for Account 9), `/pr2-backend/.../purchaseorder/**`, `/pr2-backend/.../goodsreceipt/**`
**When to start:** Day 2, as soon as P4.2 and P3.2 land. This is the PR2-side mirror of Account 5 — the deterministic core of Combination 4's procurement half.
**Expected output:** Day 2 — a documented, unit-tested weighted supplier-scoring engine that visibly favors the seeded "expensive but reliable" supplier over the "cheap but unreliable" one, plus both requisition-creation paths (manual and the `/from-recommendation` receiver Account 5 will call). Day 3 — PO auto-generation wired to the scoring engine, and simulated goods receipt with correct PO status transitions (`ISSUED`→`RECEIVED`/`PARTIALLY_RECEIVED`). Day 4 late — expanded edge-case tests for supplier scoring (capacity exclusion, ties) and the 3-way-match engine.
...
**Role:** Gemini NLP
**Prompts:** P12.1, P28.2
**Dependencies:** P11.1 (for P12.1); P26.1/P27.1 + `DEMO_SCRIPT.md` (for P28.2)
**Files owned:** `/pr2-backend/.../ai/GeminiClient.java`, `/pr2-backend/.../requisition/IntentExtractionService.java`, `IntentExtractionResult.java`; later, `/PRESENTATION_OUTLINE.md`
**When to start:** Day 2–3, as soon as P11.1 lands. This account creates the project's single shared `GeminiClient.java` that Account 12 will reuse in Phases 16–17 — flag its completion clearly so Account 12 doesn't build a second one.
**Expected output:** Day 3 — the natural-language requisition endpoint correctly parsing the exact demo sentence ("We need 5,000 more units of MED-104 for the flu season") into a structured guess, with graceful fallback on parse failure, and the hard rule enforced in code comments that this endpoint never auto-creates a requisition. Day 5 — the presentation content outline, with this account best-placed to write the deterministic-core-vs-AI-at-the-edges section since it built the clearest example of the pattern.

---

## ACCOUNT 10
**Role:** PR2 Frontend
**Prompts:** P19.1, P19.2, P19.3
**Dependencies:** P1.2 + P11.1 (for P19.1); P19.1+P12.1+P13.1+P14.1 (for P19.2); P19.1+P16.1+P17.1+P18.1 (for P19.3)
**Files owned:** `App.tsx` (Procurement routes only — see shared-file note), `/frontend/src/pages/procurement/**`, `/frontend/src/api/pr2Client.ts`
**When to start:** Day 3, once Account 7 has landed `App.tsx`'s initial route structure and `useAuthStub.ts` is in place. Reuses Account 7's auth stub and NavBar pattern — does not duplicate them.
**Expected output:** Day 3 — the Procurement dashboard shell. Day 3 late — requisition/PO/GRN views plus the NL requisition chatbot, which is the demo's flagship "type a sentence, get a pre-filled confirm form" moment (Section 16 step 4) — must visibly require a confirm click, never auto-submit. Day 4 — invoice upload, exception queue (surfacing Gemini's plain-English mismatch explanation), and the P2P analytics view that closes out the demo.
...
**Role:** Tesseract OCR
**Prompts:** P15.1, P25.3
**Dependencies:** P4.1 (for P15.1); P20.1+P21.1+P22.1+P18.1+P19.3 — i.e., the whole system functional (for P25.3)
**Files owned:** `/p1-backend/src/services/ocrService.ts`, `/p1-backend/src/routes/internalOcrRoutes.ts`; later, `/scripts/e2e_smoke_test.ts`, `/scripts/README_smoke_test.md`
**When to start:** Day 2, as soon as P4.1 lands.
**Expected output:** Day 2 — a working internal `POST /internal/ocr/extract` endpoint (internal-key protected) returning legible raw text from all 3 sample invoices, unblocking Account 12's Phase 16. This account is then idle Day 2 late through Day 3 — use that window to pre-read Section 16's demo script and start drafting the smoke-test step list so Day 4's P25.3 moves fast. Day 4 — the single automated script that walks all 10 Section 16 demo steps against the live system and is the primary tool Account 15 uses to validate integration on Day 5.

---

## ACCOUNT 12
**Role:** 3-Way Matching / Invoicing / Payment
**Prompts:** P16.1, P17.1, P18.1
**Dependencies:** P14.1 + P15.1 (for P16.1); P16.1 (for P17.1); P17.1 (for P18.1)
**Files owned:** `/pr2-backend/.../invoice/**` (Invoice entity, `OcrClient`, `InvoiceStructuringService`, `InvoiceController`, `ThreeWayMatch` entity, `ThreeWayMatchEngine`, `MismatchExplanationService`, `ThreeWayMatchController`), `/pr2-backend/.../payment/**`
**When to start:** Day 3, once P14.1 (goods receipt) and P15.1 (OCR) both land. This account owns the longest sequential chain in PR2 (invoice → match → payment) — plan for it to run through most of Day 3 into Day 4 without much idle time.
**Expected output:** Day 3 — invoice upload calling Account 11's OCR endpoint then Account 9's Gemini client to structure raw text into fields, persisting even on partial failure. Day 3 late — a deterministic, Gemini-independent 3-way match engine (documented tolerance, e.g. 2%) correctly classifying all 3 sample invoices, with a Gemini-generated plain-English explanation generated only on mismatch. Day 4 — auto-approve on match / exception-queue routing on mismatch, completing the full PR2 backend pipeline described in Section 4.

---

## ACCOUNT 13
**Role:** Supabase Realtime
**Prompts:** P21.1
**Dependencies:** P2.1, P9.2, P19.2, P19.3
**Files owned:** `/frontend/src/hooks/useRealtimeTable.ts`, `/frontend/src/lib/supabaseClient.ts`; small additive hook-usage edits inside `RecommendationsView.tsx`, `RequisitionsView.tsx`, `InvoiceUploadView.tsx`, `ExceptionQueueView.tsx`
**When to start:** Day 3–4, once Accounts 7 and 10 have landed the four views this account wires up. This account has no work before then — use the idle Day 1–2 window to review Section 8/21's realtime design and confirm with Account 2 which tables have replication enabled, so there's zero ambiguity when P9.2/P19.2/P19.3 land.
**Expected output:** Day 4 — all four dashboard views auto-refreshing on live Postgres changes with no manual page reload, using only the public anon key (never the service-role key) on the frontend — the demo's key "wow" moment for judges (Section 16's realtime callouts at steps 2–3 and 7–9).

---

## ACCOUNT 14
**Role:** Authentication / Frontend Hardening
**Prompts:** P23.1, P23.2, P23.3, P24.3
**Dependencies:** P2.1+P9.1+P19.1 (for P23.1); P2.1+P4.1 (for P23.2); P2.1+P4.2 (for P23.3); P9.2+P19.2+P19.3 (for P24.3)
**Files owned:** `/frontend/src/pages/LoginPage.tsx`, `useAuth.ts`, `RequireRole.tsx`; `/p1-backend/src/auth/verifySupabaseJwt.ts`; `/pr2-backend/.../security/**`; `/frontend/src/components/ErrorBoundary.tsx`, `Toast.tsx`, `useToast.ts`, `/frontend/src/api/apiInterceptor.ts`
**When to start:** Day 4, once both frontend shells (P9.1/P19.1) exist for P23.1, and independently Day 2–3 for the backend JWT-verification halves (P23.2/P23.3), which only need P4.1/P4.2. Split this account's own time: backend JWT middleware can start as early as Day 2–3 in parallel with frontend feature work; the frontend auth swap and error-boundary work should wait until Day 4 so it lands after the real dashboard pages exist.
**Expected output:** Day 2–3 — both backends returning 401 on missing/invalid tokens and 200 on valid Supabase session tokens, with the P1→PR2 internal handoff route and the OCR internal route both correctly exempted from user-JWT checking. Day 4 — real login/logout/role-gating replacing the stub (single import-line swap in downstream files, per the stub's frozen contract), plus a global error boundary and toast system so no backend failure is ever silent during the live demo.

---

## ACCOUNT 15
**Role:** Integration / Deployment
**Prompts:** P26.1, P27.2, P28.1
**Dependencies:** P25.3 (for P26.1); P26.1 (for P27.2); P26.1+P27.1 (for P28.1)
**Files owned:** `/INTEGRATION_BUG_BASH.md`; `/infra/render.yaml` or `/infra/railway.json`, `/DEPLOYMENT.md`; `/scripts/reset_demo_data.ts`, `/DEMO_SCRIPT.md`
**When to start:** Day 4 late / Day 5, once Account 11's P25.3 smoke test exists and passes at least once.
**Expected output:** Day 5 morning — a triaged bug log (Critical/Major/Minor) with every Critical item either fixed in a narrow diff or handed back to its owning account by name, and a clean final smoke-test run. Day 5 midday — optionally, a public Render/Railway URL for judges to browse pre-demo (best-effort, documented blocker if it doesn't land in time). Day 5 afternoon — the one-command demo-reset script and the literal presenter's script mirroring Section 16's 10 steps, rehearsed at least once end-to-end.

---

## 5-DAY SCHEDULE

| Day | Accounts active | Milestone at end of day |
|---|---|---|
| **Day 1** | 1 (kickoff), 2, 4, 7 (+ 3/5/6/8/9/10/11/12/13/14 idle or reading context docs) | Repo skeleton exists; both backend and frontend scaffolds boot with `/health`/placeholder page; Supabase schema is live with RLS + Realtime enabled. |
| **Day 2** | 3, 4 (foundation), 5, 8, 9, 11, 14 (backend JWT) | Both schemas seeded and demo-ready; P1 and PR2 backends wired to Supabase with published/mirrored contract; P1's demand + inventory/expiry engines done; PR2's supplier scoring + requisition entity done; OCR endpoint live; Gemini intent-extraction endpoint live; both backends' JWT middleware verifying tokens. |
| **Day 3** | 5 (FEFO/ROP/EOQ + recommendation gen), 6, 7, 8 (PO/GRN), 9 idle window, 10, 12, 13 (prep only) | P1's full deterministic core complete and exposed over REST; Planner dashboard live with real data; PR2's PO/GRN/invoice-upload/3-way-match pipeline complete; Procurement dashboard shell + requisition/PO/GRN views + NL chatbot live. |
| **Day 4** | 5 (handoff + rationale), 10 (finish), 11 (smoke test), 12 (payment), 13, 14 (frontend auth + hardening), 24.x accounts (4/14) | Live P1→PR2 handoff and AI rationale text working; PR2 payment/exception flow complete; PR2 frontend fully wired (invoice upload, exceptions, analytics); both dashboards update in realtime; real login/role-gating live on both dashboards; error boundaries + toasts live; error handling hardened on both backends; unit-test gap-fill done; end-to-end smoke test script passing. |
| **Day 5** | 15, 1 (P27.1), 9 (P28.2) | Full Section 16 demo flow rehearsed and passing; Critical bugs resolved; `docker compose up` works clean; README finalized; demo-reset script + presenter script ready; presentation outline complete. **Ship.** |

---

## FINAL INTEGRATION CHECKLIST

- [ ] `docker compose up` brings up all three containers (`frontend`, `p1-backend`, `pr2-backend`) on a clean checkout with a correctly filled `.env` (Account 1/15).
- [ ] All tables from Section 7.1/7.2 exist in Supabase with RLS and Realtime enabled on the six required tables (Account 2).
- [ ] Seed data loads cleanly and produces a non-empty, demo-ready dashboard on first load, including the 3–5 pre-seeded low-stock alerts (Account 3).
- [ ] Every deterministic calculation (demand, safety stock, EOQ, ROP, FEFO, expiry risk, supplier scoring, 3-way match) is a pure function with zero AI calls in its code path — verified by code review, not just by testing (Accounts 5, 8, 12; audited in P26.1).
- [ ] The P1→PR2 handoff works live end-to-end: a HIGH/CRITICAL recommendation created in P1 produces a matching SYSTEM-sourced requisition in PR2 within seconds; a LOW-urgency one does not (Account 5's P20.1, verified in P25.3/P26.1).
- [ ] AI (Gemini) is used only for: (a) NL requisition intent extraction pre-fill, (b) rationale/explanation text after numbers are already computed, (c) invoice OCR-text-to-JSON structuring — no AI output feeds a quantity/threshold/decision field anywhere (Accounts 9, 5, 12; audited in P26.1).
- [ ] Both dashboards reflect state changes in realtime via Supabase Realtime, using only the anon key on the frontend (Account 13).
- [ ] Authentication works for all three roles (`PLANNER`, `PROCUREMENT_OFFICER`, `ADMIN`); role-gated routes are enforced server-side, not just hidden in the UI (Account 14).
- [ ] A simulated Gemini rate-limit/failure and a simulated OCR failure both degrade gracefully (documented fallback text / clear typed error), never a crash or a silent drop (Accounts 4, 5, 9, 12).
- [ ] The full 10-step demo flow (Section 16) runs live without any manual database edits (Account 15's P25.3/P26.1/P28.1).
- [ ] No secrets are committed anywhere in the repo; `.env.example` is complete and audited against every actual `process.env.*`/`@Value`/`import.meta.env.*` usage (Account 1).
- [ ] `INTEGRATION_BUG_BASH.md` shows zero unresolved Critical findings (Account 15).

---

## FINAL DEMO CHECKLIST

- [ ] `reset_demo_data.ts` has been run and the system is in a fresh, demo-ready state (master data intact, no leftover rehearsal transactional data) (Account 15).
- [ ] Demo accounts work: `planner@demo.com`, `procurement@demo.com`, `admin@demo.com` / `demo1234` (Accounts 3, 14).
- [ ] Step 1 — Planner Dashboard shows current inventory, expiry-risk heatmap, and pre-seeded low-stock alerts on first load with no manual action (Accounts 3, 7).
- [ ] Step 2 — Demand Spike Simulator produces a new recommendation with a visible AI rationale, live on screen without a manual refresh (Accounts 5, 7, 13).
- [ ] Step 3 — The new recommendation appears as a system-generated Purchase Requisition on the Procurement Dashboard, live (Accounts 5, 10, 13).
- [ ] Step 4 — The NL requisition chatbot correctly parses the demo sentence into a pre-filled, human-confirmed form (Accounts 9, 10).
- [ ] Step 5 — Supplier selection and PO auto-creation show the price/lead-time/OTD/quality score breakdown, not an AI decision (Accounts 8, 10).
- [ ] Step 6 — Goods receipt is a working one-click "Mark as Received" action (Accounts 8, 10).
- [ ] Step 7 — Uploading the matching sample invoice shows Gemini-extracted structured fields (Accounts 9, 12, 10).
- [ ] Step 8 — Running the 3-way match on the matching invoice shows MATCHED → payment auto-approved, and the P2P analytics touchless % ticks up (Account 12, 10).
- [ ] Step 9 — Uploading a mismatch sample shows MISMATCHED, lands in the Exception Queue with a Gemini plain-English explanation, and can be manually resolved (Accounts 12, 10).
- [ ] Step 10 — The P2P analytics dashboard closes the loop with touchless-processing rate, cycle time, and a clear tie-back to the original demand signal (Account 10).
- [ ] `DEMO_SCRIPT.md` presenter notes have been read through once live, including the "if something breaks" contingency line for each step (Account 15, presenter).
- [ ] `PRESENTATION_OUTLINE.md` explicitly hits every judged criterion from the hackathon deck, and calls out the P1→PR2 integration and the deterministic-core/AI-at-the-edges design as the two headline differentiators (Account 9).

---

*End of PROMPTS.md — 45 prompts across 28 phases, allocated across 15 parallel Claude accounts in the Account Execution Plan above. No application source code was written in producing this file, per instructions.*
